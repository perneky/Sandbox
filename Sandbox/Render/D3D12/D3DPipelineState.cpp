...
D3DPipelineState::D3DPipelineState( const PipelineDesc& desc, D3DDevice& device, const wchar_t* debugName )
{
  ...
  psoDesc.RasterizerState.MultisampleEnable     = desc.samples > 1 ? TRUE : FALSE;
  psoDesc.RasterizerState.AntialiasedLineEnable = FALSE;
  psoDesc.RasterizerState.ForcedSampleCount     = 0;
  psoDesc.RasterizerState.ConservativeRaster    = desc.rasterizerDesc.conservative ? D3D12_CONSERVATIVE_RASTERIZATION_MODE_ON : D3D12_CONSERVATIVE_RASTERIZATION_MODE_OFF;
  ...
  eastl::vector< D3D12_INPUT_ELEMENT_DESC > currentInputElements;
...
ID3D12PipelineState* D3DPipelineState::GetD3DPipelineState() const
{
  return d3dPipelineState;
}

ID3D12RootSignature* D3DPipelineState::GetD3DRootSignature() const
{
