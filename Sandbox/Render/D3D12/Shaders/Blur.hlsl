...
Texture2D< half4 > InputBuf : register( t0 );
RWTexture2D< half4 > Result : register( u0 );
...
void BlurVertically( uint2 pixelCoord, uint topMostIndex )
{
    ...
    Result[ pixelCoord ] = half4( BlurPixels( s0, s1, s2, s3, s4, s5, s6, s7, s8 ), 1 );
}
...
void main( uint3 Gid : SV_GroupID, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID )
{
    ...
    //
    uint2 GroupUL = (Gid.xy << 3) - 4;                // Upper-left pixel coordinate of group read location
    uint2 ThreadUL = (GTid.xy << 1) + GroupUL;        // Upper-left pixel coordinate of quad that this thread will read

    //
    // Store 4 unblurred pixels in LDS
    //
    uint destIdx = GTid.x + (GTid.y << 4);
    Store2Pixels(destIdx+0, InputBuf[ThreadUL + uint2(0, 0)].rgb, InputBuf[ThreadUL + uint2(1, 0)].rgb);
    Store2Pixels(destIdx+8, InputBuf[ThreadUL + uint2(0, 1)].rgb, InputBuf[ThreadUL + uint2(1, 1)].rgb);
