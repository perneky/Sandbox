...
public:
  ...
  D3DResource( D3DDevice& d3dDevice
  ...
  void EndFeedback( CommandQueue& commandQueue, Device& device, CommandList& commandList, uint64_t fence, uint64_t frameNo, int globalFeedback ) override;
...
protected:
  ...
  bool ManageTile( D3DDevice& d3dDevice, D3DCommandList& d3dCommandList, int tx, int ty, int mipLevel, uint64_t frameNo, int globalFeedback );

  void DropTile( D3DDevice& d3dDevice, D3DCommandList& d3dCommandList, TileStats& stats );

  void UpdateIndex( Device& device, CommandList& commandList, const TileStats& stats );
