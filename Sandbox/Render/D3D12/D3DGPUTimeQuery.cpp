...
D3DGPUTimeQuery::D3DGPUTimeQuery( D3DDevice& device )
{
  ...
  resultBuffer.reset( static_cast< D3DResource* >( device.CreateBuffer( ResourceType::Buffer, HeapType::Readback, false, sizeof( uint64_t ), sizeof( uint64_t ), L"TimestampQueryBuffer" ).release() ) );
}
